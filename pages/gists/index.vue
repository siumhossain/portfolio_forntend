<template >
        ...
        <div>
         ...
         <header>
    <div>
      <h1 @click='brand()' class="mx-2 my-2 display-5 brand">Sium</h1>
      <div class='search-box'>
        
          <input type="search" name="search" v-on:keyup.enter="submit()" v-model="search"   class="search-prompt">
  ...
  <main>
    <!-- search results -->
    <p class="results-returned">About {{Object.keys(blogObj).length}} results</p>

    <ul>
      <li v-for="i in blogObj" :key="i.id">
        
            <Nuxt-link :to="{ path:`gists/${i.id}`}">
            <h2 class="result-link">{{i.title}}</h2>
            </Nuxt-link>
